...
def generate_plan(
    ...
    history_text = ""
    if history:
        recent_history = history[-6:]  # Last 3 exchanges (user + assistant)
        history_lines = []
        for item in recent_history:
            if item['role'] == 'user':
                history_lines.append(f"User: {item['content']}")
            elif item['role'] == 'assistant':
                history_lines.append(f"Assistant: {item['content']}")
        if history_lines:
            history_text = "\n\nRecent Conversation:\n" + "\n".join(history_lines)
